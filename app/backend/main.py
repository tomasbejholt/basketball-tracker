...
async def track(
    ...
    # Pass 1 – collect raw detections via ByteTrack (runs in thread to keep event loop free)
    raw_positions, raw_boxes = await asyncio.to_thread(
        _run_inference, model, input_path, conf, job_id
    )

    # Reset tracker state so next request starts clean
    ...
    # Interpolate gaps ≤ half a second, then smooth
